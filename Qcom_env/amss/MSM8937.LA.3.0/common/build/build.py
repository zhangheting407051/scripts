...
-------------------------------------------------------------------------------
  ...
  $Header: //components/rel/build.glue/1.0/build.py#13 $
  $DateTime: 2015/10/28 16:01:58 $
...
# Function to create  Product_Info.json file 
...
def evaluate_params (step_dict_flavor, params = None):
   ...
   for i in range(len(params_split)):
      ...
      #zhangheting@wind-mobi.com mod 20171023 begin, NO need to copy A out/ to N code
      if params_split[i].startswith('LINUX/android/out'):
         #params_split[i] = params_split[i].replace("LINUX/android/out/", "../../out/")
		 params_split[i] = "../../out/target/product/"+sys.argv[1]+"/"
      #zhangheting@wind-mobi.com mod 20171023 end
   new_params_split = []
   for i in range(len(params_split)):
      if params_split[i] == '+':
...
parser.add_option("--product_json",action="store",dest="product_json",help="Create Product Json",default=False)
